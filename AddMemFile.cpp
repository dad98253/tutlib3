...
#ifdef DEBUG
	dfprintf2(__LINE__,__FILE__,TRACE,"in AddMemFile, szFileName = %s, iFileSize = %i, iOption = %i\n",szFileName,iFileSize,iOption);
#endif
...
//	#define DIRSIZE		1
...
#ifdef DEBUG
		dfprintf2(__LINE__,__FILE__,TRACE,"in AddMemFile, FileNameList[%i] = \"%s\", szFileName = \"%s\"\n",i,FileNameList[i],szFileName);
#endif
...
#ifdef DEBUG
	dfprintf2(__LINE__,__FILE__,TRACE,"in AddMemFile, offset for %s = %u bytes\n",szFileName,TotalMemFileSystemSize);
#endif
	...
	if ( iOption == 1 ) {
...
#ifdef DEBUG
			dfprintf2(__LINE__,__FILE__,TRACE,"memFile is full... compressed file size = %u, TotalMemFileSystemSize = %u, uncomprMemFileSystemLen = %u\n",comprLen,TotalMemFileSystemSize,uncomprMemFileSystemLen);
#endif
			return(-20);
		}
		...
		free(compr);
#ifdef DEBUG
		dfprintf2(__LINE__,__FILE__,TRACE,"compressed file size = %u, file saved as directory entry number %i\n",comprLen,iFileNumber);
#endif
	} else {
		if ( (TotalMemFileSystemSize+iFileSize) > uncomprMemFileSystemLen ) {
#ifdef DEBUG
			dfprintf2(__LINE__,__FILE__,TRACE,"memFile is full... file size = %i, TotalMemFileSystemSize = %u, uncomprMemFileSystemLen = %u\n",iFileSize,TotalMemFileSystemSize,uncomprMemFileSystemLen);
#endif
			return(-20);
		}
...
#ifdef DEBUG
		dfprintf2(__LINE__,__FILE__,TRACE,"file saved as directory entry number %i\n",iFileNumber);
#endif
	}
#ifdef DEBUG
		dfprintf2(__LINE__,__FILE__,TRACE,"TotalMemFileSystemSize is now %u\n",TotalMemFileSystemSize);
#endif
		iFileNumber++;
