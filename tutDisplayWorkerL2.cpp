...
#ifdef DEBUG
		if (debugflag ) dfprintf(fp9,"FindTutDPRecord failed at line %i in tutDisplayWorker, iRet = %i\n",__LINE__,iRet);
#endif
...
#ifdef DEBUG
		if (debugflag ) dfprintf(fp9,"ReplaceText2 failed at line %i in tutDisplayWorker, iRet = %i\n",__LINE__,iLenResource);
#endif
		return(RET_ABORTPROGRAM);
	}
#ifdef DEBUG
//	if (debugflag ) dfprintf(fp9,"iDisplayPointID = %i looks like this after string replacement in %s :\n%s",iDisplayPointID,__MYFILE__,lpsEditedResource);
...
#ifdef DEBUG
		if (debugflag ) dfprintf(fp9,"ParseDPRecord failed at line %i in tutDisplayWorker, iRet = %i\n",__LINE__,iRet);
#endif
		return (RET_ABORTPROGRAM);
	}
...
#ifdef DEBUG
	if (debugflag ) dfprintf(fp9,"tutDisplayWorker found valid input at line %i, iRet = %i\n",__LINE__,iRet);
#endif
