...
//		return(-3);
   ...
   // check to make sure we have the correct ZLIB version...
...
#ifdef DEBUG
		dfprintf2(__LINE__,__FILE__,TRACE, "incompatible zlib version (\"%s\")\nthis is a programming error...\n",myzlibVersion);
	...
	if (strcmp(zlibVersion(), myzlibVersion) != 0) {
		dfprintf2(__LINE__,__FILE__,TRACE, "warning: different zlib version (\"%s\")\nthis is a programming problem...\n",myzlibVersion);
	}
	dfprintf2(__LINE__,__FILE__,TRACE,"zlib version %s = 0x%04x, compile flags = 0x%lx\n",ZLIB_VERSION, ZLIB_VERNUM, zlibCompileFlags());
#endif
...
#ifdef DEBUG
	dfprintf2(__LINE__,__FILE__,TRACE, "MemFile format = 0x%x\n",bOneByte);
#endif
	if ( bOneByte != bMemFileFormat ) {
#ifdef DEBUG
		dfprintf2(__LINE__,__FILE__,TRACE, "MemFile format is incomatible! Expected format == 0x%x\n",bMemFileFormat);
#endif
		return(-21);
	}
...
#ifdef DEBUG
	dfprintf2(__LINE__,__FILE__,TRACE, "MemFile size of short int = 0x%x\n",bShortIntSize);
#endif
	if ( bShortIntSize != sizeof(short unsigned int) ) {
#ifdef DEBUG
		dfprintf2(__LINE__,__FILE__,TRACE, "MemFile format is incomatible! Expected short int size == 0x%x\n",sizeof(short unsigned int));
#endif
		return(-22);
	}
...
#ifdef DEBUG
	dfprintf2(__LINE__,__FILE__,TRACE, "MemFile size of int       = 0x%x\n",bIntSize);
#endif
	if ( bIntSize != sizeof(unsigned int) ) {
#ifdef DEBUG
		dfprintf2(__LINE__,__FILE__,TRACE, "MemFile format is incomatible! Expected int size == 0x%x\n",sizeof(unsigned int));
#endif
		return(-23);
	}
...
#ifdef DEBUG
	dfprintf2(__LINE__,__FILE__,TRACE, "MemFile size of long int  = 0x%x\n",bLongIntSize);
#endif
	if ( bLongIntSize != sizeof(long unsigned int) ) {
#ifdef DEBUG
		dfprintf2(__LINE__,__FILE__,TRACE, "MemFile format is incomatible! Expected long int size == 0x%x\n",sizeof(long unsigned int));
#endif
		return(-24);
	}
...
//		dfprintf2(__LINE__,__FILE__,TRACE,"in LoadMemFile, found \"%s\", DirSize = %u, DirOffset = %u, expected directory offset = %i\n",szFileName,DirSize,DirOffset,ijkTemp);
		dfprintf2(__LINE__,__FILE__,TRACE,"in LoadMemFile2, looking for \"%s\", DirSize = %u, DirOffset = %u\n",szFileName,DirSize,DirOffset);
	} else {
		dfprintf2(__LINE__,__FILE__,TRACE,"in LoadMemFile2, DirSize = %u, DirOffset = %u, szFileName = \"%s\"\n",DirSize,DirOffset,szFileName);
	}
#endif
