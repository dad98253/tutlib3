...
#ifdef DEBUG
		if ( debugflag && ( strlen(szText) == 0 || !bOutputDP ) ) dfprintf(fp9,"tutPrintString called with bOutputDP = %u and string = \"%s\" (length %i)\n",bOutputDP,szText,strlen(szText));
		if ( debugflag && strlen(szText) != 0 && bOutputDP ) dfprintf(fp9," %s\n",szText);
#endif
