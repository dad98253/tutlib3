...
#ifdef DEBUG
	if (getflag(TUTINDEXDBDBG) ) dfprintf(fp9,"in tutIndexTutdb, initial value of uncomprLen = %lu\n",uncomprLen);
#endif
...
#ifdef DEBUG
	if (getflag(TUTINDEXDBDBG) ) dfprintf(fp9,"the actual start of the tutor data records is at %u (should be %u)\n",TotalFileSize-iInitailFileOffset,sizeof(short unsigned int));
#endif
	while(1) {
...
#ifdef DEBUG
			if (getflag(TUTINDEXDBDBG) ) dfprintf(fp9,"iTotRecSize = %u, index offset (rel to start of index) = %u\n",iTotRecSize,TotalIndexSize);
#endif
...
#ifdef DEBUG
		if (getflag(TUTINDEXDBDBG) ) dfprintf(fp9,"last record saved... iTotRecSize = %u, index offset (rel to start of index) = %u\n",iTotRecSize,TotalIndexSize);
#endif
		iTotRecSize = 0;
...
#ifdef DEBUG
	if (getflag(TUTINDEXDBDBG) ) dfprintf(fp9,"TotalFileSize = %lu, TotalIndexSize = %lu, iRecNumber = %i\n",TotalFileSize-iInitailFileOffset,TotalIndexSize,iRecNumber);
	if (getflag(TUTINDEXDBDBG) ) dfprintf(fp9,"calculated  TutorIndexOffset = %u\n",*TutorIndexOffset);
#endif
...
#ifdef DEBUG
	if (getflag(TUTINDEXDBDBG) ) dfprintf(fp9,"total size of tutor file (uncompressed) = %lu\n",uncomprLen);
#endif
