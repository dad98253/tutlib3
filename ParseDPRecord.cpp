...
#ifdef DEBUG
		if (debugflag ) dfprintf(fp9,"malloc for szDPText in ParseDPRecord failed at line %i\n",__LINE__);
#endif
		return(-1);
	}
...
#ifdef DEBUG
	if (debugflag) {
		dfprintf(fp9,"\33[1;35mparse of display point data\33[0m\n");
		...
		if (*szDPIDString != NULL)dfprintf(fp9,"Display Point ID = \"%s\"\n",*szDPIDString);
		dfprintf(fp9,"%s number of %s fields, %i allowed\n",szftxed,szdatatype,DPInfo->iNumFieldsAllowed);
		if (DPInfo->iFieldNumber>3)dfprintf(fp9,"valid inputs: %i %s %i\n",DPInfo->iStartRange,szTO,DPInfo->iEndRange);
		if (*szDefaultInput != NULL)dfprintf(fp9,"default input = \"%s\"\n",*szDefaultInput);
	}
#endif
